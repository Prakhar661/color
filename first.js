...
document.body.addEventListener("click", (event)=>{
  ...
  rectangle.textContent="Hi";
  
  const x = event.clientX;
  const y = event.clientY;

  rectangle.style.left = `${x-25}px`
  rectangle.style.top= `${y-25}px`


  const color = ["red", "blue", "orange", "green","purple", "white","yellow", "wheat"];
  rectangle.style.backgroundColor = color[Math.floor(Math.random()*color.length)];
